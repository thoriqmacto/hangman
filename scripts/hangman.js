// ----- Global variables ----- 
// Instead of using jquery DOM selector $(), we used the standard JavaScript getElementById() method, 
// because the reference to the <canvas> element needs to be a std JavaScript object, not a jQuery object.
...
function init(){
	...
	$('#loading').hide();
	$('#play').css('display', 'inline-block').click(function(){
	...
	// Display help info when clicking
	...
	// If it’s 700 pixels or more, the <canvas> element’s drawing context is scaled horizontally and vertically 
	// by a ratio of 1.5—in other words, increased by 50 percent.
...
function newGame(){
	...
	// Using standard JavaScript innerHTML property to insert the underscores in the word paragraph. You need to use innerHTML rather than the jQuery html() method because word is one of the global variables stored as a standard JavaScript object.
	word.innerHTML = placeholders;
	
	// create 26 alphabet pad to select letters
	letters.innerHML = '';
	for (i = 0; i < 26; i++){
...
function getLetter(){
	...
	// remove it after clicking and set the cursor to default
	this.innerHTML = '&nbsp;';
	this.style.cursor = 'default';
	
	// remove event handler getLetter()
	this.onclick = null;	
}
...
function drawCanvas(){
	...
	if (correctGuesses == wordLength){
		...
		// increase score of won games, display score
